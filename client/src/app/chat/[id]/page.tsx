...
import wsClient from "@/lib/websocket";
import { Channel, WSEvent } from "@/lib/types";
>>>>>>> afe26d853ebee6c1874eeead6fa5aeaf948654da
...
<<<<<<< HEAD
        oldestMessageIdRef.current = displayMsgs.length > 0
          ? displayMsgs[0].id
          : null;
      } catch (e: any) {
        ...
        case 'user_offline':
          if (event.data?.user_id)
            setOnlineUserIds(prev => {
              const s = new Set(prev); s.delete(event.data.user_id); return s;
=======
        case "online_users":
          setOnlineUserIds(
...
<<<<<<< HEAD
  ...
  const handleDeleteChannel = useCallback(async (channelId: string) => {
    try {
      ...
      alert('Erreur: ' + (e?.message || 'Impossible de supprimer le channel'));
    }
  }, [selectedChannel]);
...
const handleKick = useCallback(
    async (userId: string) => {
      try {
        const token = getCurrentUser()?.token || localStorage.getItem("token");
        const response = await fetch(
          `http://localhost:3001/servers/${serverId}/members/${userId}/kick`,
          {
            method: "DELETE",
            headers: {
        ...
        if (!response.ok) throw new Error("Kick échoué");
        // Retirer le membre de la liste localement
        setMembers((prev) => prev.filter((m) => m.id !== userId));
      } catch (e: any) {
        alert("Erreur: " + (e?.message || "Impossible de kicker ce membre"));
      }
    },
    [serverId]
  );
      ...
      <div className="chat-zone">
        ...
        <div className="chat-header">
          ...
          <div className="chat-header-right" style={{ position: 'relative', display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <button
              ...
              style={{
                cursor: 'pointer', fontSize: '13px', background: '#248046', color: '#fff',
                border: 'none', padding: '4px 12px', borderRadius: '4px', fontWeight: 600,
              }}
            >
              👤+ Inviter
            </button>
            ...
            {showSettings && (<React.Fragment>
              ...
              <div style={{
                position: 'absolute', top: '110%', right: 0, background: '#111214',
                border: '1px solid #3f4147', borderRadius: '8px', padding: '6px 0',
                minWidth: '220px', zIndex: 100, boxShadow: '0 8px 24px rgba(0,0,0,0.6)',
              }}>
                {(myRole === 'owner' || myRole === 'admin') && (
                  <button
                    ...
                    style={{
                      ...
                      border: 'none', color: '#ed4245', textAlign: 'left', cursor: 'pointer', fontSize: '14px',
                    }}
            ...
            {showInviteModal && (<React.Fragment>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '12px', color: '#b5bac1', marginBottom: '8px', fontWeight: 700, textTransform: 'uppercase' }}>
                  Code d&apos;invitation
                </div>
                {inviteCode ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#1e1f22', borderRadius: '6px', padding: '8px 12px' }}>
                    <code style={{ flex: 1, color: '#00b0f4', fontSize: '16px', fontWeight: 600, letterSpacing: '1px', userSelect: 'all' }}>
                      {inviteCode}
                    </code>
                    <button
                      onClick={handleCopyInvite}
                      style={{
                        background: inviteCopied ? '#248046' : '#5865f2', color: '#fff', border: 'none',
                ...
                <div style={{ fontSize: '12px', color: '#888', marginTop: '12px' }}>
                  Partagez ce code pour inviter vos amis depuis la page Serveurs.
                </div>
              </div>
            </React.Fragment>)}
          </div>
        ...
        {(!selectedChannel || channels.length === 0) && !loading && (
          <div style={{
            ...
            color: '#4f545c', gap: '16px',
          }}>
            <div style={{ fontSize: '3rem' }}>{'📭'}</div>
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: '#6d6f78' }}>Aucun channel disponible</div>
            <div style={{ fontSize: '0.85rem', color: '#4f545c', textAlign: 'center', maxWidth: '260px' }}>
              {canManage
          ...
          <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 0' }}>
            <button
              ...
              style={{
                ...
                color: '#8e9297', borderRadius: '6px',
                padding: '6px 16px', cursor: 'pointer', fontSize: '0.8rem',
                transition: 'all 0.2s',
              }}
              onMouseEnter={e => { e.currentTarget.style.background = '#3f4147'; e.currentTarget.style.color = '#fff'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = '#8e9297'; }}
            >
              {loadingMoreMessages ? 'Chargement...' : '↑ Charger les messages précédents'}
            </button>
          </div>
        )}
        ...
        {sendError && (
          <div style={{
            ...
            background: '#2b0a0a', border: '1px solid #8b1a1a',
            borderRadius: '6px', color: '#ff4444', fontSize: '0.85rem',
          }}>
            ⚠️ {sendError}
          </div>
        )}
          ...
          {typingNames.length > 0 && (
            <div style={{ fontSize: '12px', color: '#b5bac1', fontStyle: 'italic', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span style={{ display: 'inline-flex', gap: '2px', alignItems: 'center' }}>
                {[0, 1, 2].map(i => (
                  <span key={i} style={{
                    width: '5px', height: '5px', borderRadius: '50%', backgroundColor: '#5865f2',
                    display: 'inline-block', animation: `typingBounce 1.2s ease-in-out ${i * 0.2}s infinite`,
                  }} />
                ))}
              </span>
              <span>
...
<<<<<<< HEAD
    ...
    {showOwnerLeaveModal && (
      <div style={{
        ...
        <div style={{
          background: '#2b2d31', borderRadius: '12px', padding: '32px',
          maxWidth: '420px', width: '90%', boxShadow: '0 8px 32px rgba(0,0,0,0.5)',
          border: '1px solid #3f4147',
        }} onClick={e => e.stopPropagation()}>
          <div style={{ fontSize: '2rem', marginBottom: '12px', textAlign: 'center' }}>👑</div>
          <h3 style={{ color: '#fff', margin: '0 0 12px', textAlign: 'center', fontSize: '1.1rem' }}>
            Impossible de quitter ce serveur
          </h3>
          <p style={{ color: '#b5bac1', fontSize: '0.9rem', lineHeight: '1.5', textAlign: 'center', margin: '0 0 24px' }}>
            Vous êtes <strong style={{ color: '#f0b132' }}>propriétaire</strong> de ce serveur.
            Pour le quitter, vous devez d'abord <strong style={{ color: '#fff' }}>transférer la propriété</strong> à un autre membre via le menu ⋯ dans la liste des membres.
          </p>
          <button
            onClick={() => setShowOwnerLeaveModal(false)}
            style={{
